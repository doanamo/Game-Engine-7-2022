...
{
    /*
        Retrieve engine and game directory overrides from working directory. When application is
        launched from packaged release build these files will not be present as both files would
        otherwise point at same current working directory, but when launching from generated project
        in development both will be created by CMake in temporary output directory to account for
        engine and game directories being in separate locations, same as for their repositories.
